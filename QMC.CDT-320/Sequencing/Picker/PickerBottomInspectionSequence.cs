...
namespace QMC.CDT320.Sequencing
{
    internal sealed class PickerBottomInspectionSequence : PickerSequenceBase<PickerBottomInspectionStep>
    {
        ...
        private int SelectNextPicker()
        {
            ...
            _targetPickerX = GetPickerTeachingPosition(PickerAxis.PickerX, "DieBottomPosition[" + _currentPickerIndex + "]") +
                ResolvePickerAlignOffsetX(_currentPickerIndex);
            _targetPickerY = GetPickerTeachingPosition(PickerAxis.PickerY, "DieBottomPosition[" + _currentPickerIndex + "]") +
        ...
        private async Task<int> RequestBottomInspectionAsync(CancellationToken ct)
        {
            try
            {
                ...
                return Fail("PICKER-BOTTOM-VISION-NG", "Vision", "Bottom inspection failed. die=" + _currentDie.DieId + ", pickerNo=" + _currentPickerNo);
            }
            catch (OperationCanceledException)
            {
                throw;
        ...
        private int ApplyBottomInspectionResult()
        {
            ...
            DieResult dieResult = _bottomResult.IsOk
                ? DieResult.Good
                : DieResult.NG;
        ...
        private async Task<BottomVisionOffset> RequestBottomResultAsync(CancellationToken ct)
        {
            ...
            BottomVisionOffset[] results = inspection != null ? inspection.Item1 : null;
