...
{
    internal enum OutputFeederExchangeStep { Idle, CheckUnit, UnloadCurrentToCassette, LoadNextFromCassette, MoveExchangePosition, Complete, Error }

    internal sealed class OutputFeederExchangeSequence : OutputFeederSequenceBase<OutputFeederExchangeStep>
    {
        public OutputFeederExchangeSequence(MachineSequenceContext context) : base(context, OutputFeederSequenceKind.Exchange, "OutputFeederExchangeSequence") { }
        ...
        protected override Task<int> ExecuteCurrentStepAsync(CancellationToken ct)
        {
            try
            {
                ...
                switch (CurrentStep)
                {
                    case OutputFeederExchangeStep.CheckUnit: return Task.FromResult(CheckUnit(OutputFeederExchangeStep.UnloadCurrentToCassette));
                    case OutputFeederExchangeStep.UnloadCurrentToCassette: return UnloadCurrentToCassetteAsync(ct);
                    case OutputFeederExchangeStep.LoadNextFromCassette: return LoadNextFromCassetteAsync(ct);
                    case OutputFeederExchangeStep.MoveExchangePosition: return MoveExchangePositionAsync(ct);
                    default: return Task.FromResult(FailUnsupportedStep());
                }
            }
            catch (Exception ex) { return Task.FromResult(Fail("OUT-FEEDER-EXCHANGE-EX", Name, "Exchange step failed: " + ex.Message)); }
        }
        ...
        private async Task<int> UnloadCurrentToCassetteAsync(CancellationToken ct)
        {
            ...
            if (result != 0) return Fail("OUT-FEEDER-EXCHANGE-UNLOAD", Feeder.Name, "Output feeder exchange unload failed. result=" + result);
        ...
        private async Task<int> LoadNextFromCassetteAsync(CancellationToken ct)
        {
            ...
            if (result != 0) return Fail("OUT-FEEDER-EXCHANGE-LOAD", Feeder.Name, "Output feeder exchange load failed. result=" + result);
