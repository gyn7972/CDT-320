...
namespace QMC.CDT_320.Ui.Controls
{
    ...
    /// LotStorage.ActiveLot 의 BinDistribution 을 1초마다 폴링해 셀 색상으로 시각화한다.
    /// 활성 Lot 이 없으면 "(no active lot)" 표시.
    /// </summary>
    public class LiveLotMapView : Control
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            var dmap = LotStorage.ActiveInputDieMap;   // 원형 웨이퍼 다이맵 (있으면)
            // 헤더 텍스트
            using (var bf = new SolidBrush(Color.FromArgb(0x33, 0x33, 0x33)))
            using (var f  = new Font("Consolas", 9F, FontStyle.Bold))
            {
                string head = lot != null
                    ? string.Format("LOT {0}  processed={1}/{2}  good={3}  ng={4}",
                                    lot.LotID, lot.ProcessedDies, lot.TotalDies, lot.GoodCount, lot.NgCount)
                    : "(no active lot)";
                g.DrawString(head, f, bf, 8, 4);
            }

            // 격자 크기 — 다이맵이 있으면 그 grid 사용, 없으면 default 5x5
            int gx = (dmap != null) ? dmap.GridX : _gridX;
            int gy = (dmap != null) ? dmap.GridY : _gridY;
            int margin = 8;
            int top    = 22;
            ...
            // 다이맵 모드: IsTarget=true 셀만 색칠 (원형 자동 표현)
            if (dmap != null)
            {
                Color bgInactive = BackColor;
                Color cellEmpty  = Color.FromArgb(0xCC, 0xDD, 0xEE);   // 미처리 다이 (대상)
                Color cellGood   = BinCodeMap.ConvertToBinCodeColor(BinCodeMap.GoodBin);
                Color cellNg     = Color.IndianRed;
                ...
                foreach (var entry in dmap.Entries)
                {
                    ...
                    if (!entry.IsTarget) continue;   // 원 밖 — 안 그림

                    Color c;
                    if (activeIdx < processed)
                    {
                        c = (activeIdx < good) ? cellGood : cellNg;
                    }
                    else
                    {
                        c = cellEmpty;
            ...
            // 우하단 진행률
            if (lot != null && lot.TotalDies > 0)
            {
                using (var bf = new SolidBrush(Color.FromArgb(0x33, 0x33, 0x33)))
                using (var f  = new Font("Consolas", 9F))
                {
                    string pct = string.Format("{0:F1} %", lot.ProcessedDies * 100.0 / lot.TotalDies);
