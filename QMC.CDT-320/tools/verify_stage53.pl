...
# Stage 53 — Eject + Side cycle integration in DoOneDieAsync
my $mc = "$ROOT/QMC.CDT-320/Equipment/MachineController.cs";
my $r1 = greps($mc, qr/EjectPinZ/);
row("STAGE53","MachineController DoOneDieAsync — EjectPinZ axis call",
    $r1?"PASS":"FAIL", $mc);

my $r2 = greps($mc, qr/SideVisionY/);
row("STAGE53","MachineController DoOneDieAsync — SideVisionY axis call",
    $r2?"PASS":"FAIL", $mc);
