...
my $maint = "$VIS_ROOT/Ui/Pages/MaintenancePage.cs";
my $b3 = greps($maint, qr/ParameterEditorHost/) && greps($maint, qr/ShowParameterEditors/);
row("STAGE2", "MaintenancePage → ParameterEditorHost 진입", $b3?"PASS":"FAIL", $maint);

# C. ZoomDialog
my $zd = "$VIS_ROOT/Ui/Dialogs/ZoomDialog.cs";
my $c1 = greps($zd, qr/class\s+ZoomDialog\s*:\s*Form/) &&
         greps($zd, qr/MouseWheel/) &&
         greps($zd, qr/_zoom\s*\*\s*delta/) &&
         greps($zd, qr/DoubleClick/);
row("STAGE2", "ZoomDialog — 휠 줌 + 드래그 팬 + 더블클릭 1:1", $c1?"PASS":"FAIL", $zd);
...
my $d1 = greps($cfg, qr/Cognex VisionPro diagnostics/) &&
         greps($cfg, qr/ProbeCognex/) &&
         greps($cfg, qr/CognexBackend/) &&
         greps($cfg, qr/CognexLoaded/);
row("STAGE2", "ConfigurationPage — Cognex 진단 패널 + ProbeCognex", $d1?"PASS":"FAIL", $cfg);
