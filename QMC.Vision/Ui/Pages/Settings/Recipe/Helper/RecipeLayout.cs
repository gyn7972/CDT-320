...
namespace QMC.Vision.Ui.Pages.Recipe
{
    /// <summary>
    /// 레시피/타깃 페이지 공통 레이아웃 부품 — Handler(QMC.CDT_320.Ui.Pages.Recipe.RecipeLayout) 미러.
    /// 주황 섹션바, "Label|Value" 행, 카메라 뷰, 조그 패드, 속도 슬라이더를 한 줄 호출로 동일하게 생성한다.
    /// </summary>
    internal static class RecipeLayout
    {
        ...
        /// <summary>카메라 뷰 placeholder (검정 배경 + 주황 타이틀바 + 정보 라벨).</summary>
        ...
        public static Panel SimpleJog(int x, int y, int w, int h, string axisLabel)
        {
            ...
            p.Controls.Add(OrangeBar(0, 0, w, "JOG"));
            int cx = (w - 80) / 2;
            p.Controls.Add(new Button { Location = new Point(cx, 36),  Size = new Size(80, 80), Text = "▲", Font = new Font("맑은 고딕", 18F), FlatStyle = FlatStyle.Flat });
            p.Controls.Add(new Label  { Location = new Point(cx, 120), Size = new Size(80, 24), Text = axisLabel, BackColor = Color.FromArgb(0xD0, 0xD0, 0xD0), TextAlign = ContentAlignment.MiddleCenter });
            p.Controls.Add(new Button { Location = new Point(cx, 150), Size = new Size(80, 80), Text = "▼", Font = new Font("맑은 고딕", 18F), FlatStyle = FlatStyle.Flat });
            return p;
        }
