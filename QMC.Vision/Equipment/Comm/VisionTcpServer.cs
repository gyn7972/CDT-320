...
namespace QMC.Vision.Comm
{
    ...
    public class VisionTcpServer : IDisposable
    {
        ...
        private void ProcessLine(NetworkStream stream, string line)
        {
            ...
            {
                // 응답 echo 토큰:
                //  · 동기 도구 계열(MATCH/INSPECT/TRAIN) → finder/inspector echo
                //      예: ACK|WaferVision|MATCH|AlignDieFinder|OK;x=...;y=...;r=...;score=...
                //  · 비동기 계열(MATCHASYNC/RESULT, INSPECTASYNC/RESULT) → echo 없음
                //      예: ACK|BottomInspection|MATCHASYNC|STARTED  /  ACK|BottomInspection|MATCHRESULT|1;x=..
                //    번호(chip_uid)는 echo 하지 않고 결과 저장 키로만 사용 — 핸들러가 요청한 번호와 1:1 대응.
                ...
                switch (cmd)
                {
                    case "MATCHASYNC":
                        resp = "STARTED";
                        break;   // 1차 ACK(STARTED) 후 백그라운드 알고리즘
                }
                ...
                if (resp == null) { Send(stream, $"ERR|{mod}|{cmd}|unknown command"); return; }
