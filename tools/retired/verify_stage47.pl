...
# Stage 47 — IonizerUnit
my $iu = "$ROOT/QMC.CDT-320/Equipment/IonizerUnit.cs";
my $r1 = greps($iu, qr/class\s+IonizerUnit/);
row("STAGE47","IonizerUnit.cs exists",
    $r1?"PASS":"FAIL", $iu);

my $cdtm = "$ROOT/QMC.CDT-320/Equipment/CDT320Machine.cs";
my $r2 = greps($cdtm, qr/Ionizer/);
row("STAGE47","CDT320Machine — Ionizer integration",
    $r2?"PASS":"FAIL", $cdtm);
